...
class CheckoutPage extends React.Component {
  static async getInitialProps() {
    const paymentGateways = await fetch(
      api.buildUrl({ paths: [api.WC, "payment_gateways"] })
    )
      .then(resp => resp.json())
      .then(data => {
        return data;
    ...
    const shippingMethods = await fetch(
      api.buildUrl({ paths: [api.WC, "shipping_methods"] })
    )
      .then(resp => resp.json())
      .then(data => {
        return data;
  ...
  componentDidMount(nextProps, nextState) {
    ...
    this.setState({
      customer: {
        ip: api.fetchExternalIp().then(data => {
          this.setCustomerIp(data.ip);
        }),
  ...
  submit = values => {
    ...
    let order = {
      ...
      payment_method_title: getTitleFromArray(
      ...
      shipping_lines: buildShippingLines(
        { id: values.shipping_method },
        values.shipping_method_location
      ),
