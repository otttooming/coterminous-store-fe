...
const urlParameters = api.buildUrl({ parameters: ['in_stock=true', 'status=publish', 'page=' + '1', 'per_page=16'] }, api.SITEURL);
...
const urlFull = api.buildUrl({ paths: [api.WC, 'products'], parameters: ['in_stock=true', 'status=publish', 'page=' + '1', 'per_page=16'] }, api.SITEURL);
const mockUrlFull = [
  api.SITEURL.host,
  '/wp-json/',
  `${api.WC}`,
